...
const Dashboard = () => {
  ...
  const handleSessionChange = (event) => {
    const selectedSessionID = parseInt(event.target.value);
    setSelectedSession(selectedSessionID);
  
    const selectedDateObject = sessionDates.find(date => date.id === selectedSessionID);
    setSelectedDate(selectedDateObject);
    
    if (selectedDateObject) {
      setId(selectedDateObject.id);
    }

    const transformedData = [
      {
        subject: 'Dominant', A: resultData.scoreValueD,
      },
      {
        subject: 'Invloedrijk', A: resultData.scoreValueI,
      },
      {
        subject: 'Stabiel', A: resultData.scoreValueS,
      },
      {
        subject: 'Consciëntieus', A: resultData.scoreValueC,
      },
    ];
  ...
  const fetchDates = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${config.API_BASE_URL}/api/TotalScores/user/me/dates/${selectedUser}`, { // Changed 'id' to 'selectedUser'
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      console.log("A: " + response.data.date)
      setSessionDates(Array.isArray(response.data) ? response.data : []);
    } catch (error) {
  ...
  const fetchScores = async () => {
      ...
      const token = localStorage.getItem('token');
      axios.get(`${config.API_BASE_URL}/api/TotalScores/user/me/pp/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      })
      .then(response => {
        console.log("Biem" + response.data.scoreValueD);
        ...
        <div className={styles.content}>
          ...
          <div className={styles.hm}>
            ...
            <div>
              ...
              <div>
              <select className={styles.dropdownMenu} onChange={handleSessionChange} disabled={sessionDates.length === 0}>
            <option value="">Select a session</option>
            {/* Update: Display session dates */}
            {sessionDates.map((date, index) => <option key={index} value={date.id}>{date.date}</option>)}
          </select>
              </div>
            </div>
              ...
              companies.find(c => c.id === Number(selectedCompany)).companyType !== 2 && (
                <div>
                  <RadarChart cx={300} cy={250} outerRadius={150} width={600} height={500} data={chartData}>
                    ...
                    <Radar name="Score" dataKey="A" stroke="#884d8" fill="#8884d8" fillOpacity={0.6} />
                  </RadarChart>
                  <div>
